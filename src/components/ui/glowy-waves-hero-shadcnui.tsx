...
export function GlowyWavesHero({ onGetStarted }: GlowyWavesHeroProps) {
  ...
  useEffect(() => {
    ...
    const computeThemeColors = () => {
      ...
      return {
        backgroundTop: resolveColor(["--bg"], 1),
        backgroundBottom: resolveColor(["--surface-container", "--bg"], 0.95),
        wavePalette: [
          {
            offset: 0,
            amplitude: 70,
          ...
          },
          {
            ...
            amplitude: 90,
            frequency: 0.0026,
            color: resolveColor(["--secondary", "--primary"], 0.7),
            opacity: 0.35,
          },
          {
            offset: Math.PI,
            amplitude: 60,
            frequency: 0.0034,
            color: resolveColor(["--tertiary", "--on-bg"], 0.65),
            opacity: 0.3,
          },
          {
            offset: Math.PI * 1.5,
            amplitude: 80,
            frequency: 0.0022,
            color: resolveColor(["--on-primary", "--on-bg"], 0.25),
            opacity: 0.25,
          },
          {
            offset: Math.PI * 2,
  ...
  return (
    <section
      className="relative isolate flex min-h-[90vh] sm:min-h-screen w-full items-center justify-center overflow-hidden bg-background"
      role="region"
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center px-4 sm:px-6 py-16 sm:py-24 text-center md:px-8 lg:px-12">
        <motion.div
        ...
        >
          ...
          <motion.h1
            variants={itemVariants}
            className="mb-6 text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-semibold tracking-tight text-foreground"
          >
            Turn Code into{" "}
          ...
          <motion.p
            variants={itemVariants}
            className="mx-auto mb-10 max-w-3xl text-base sm:text-lg md:text-2xl text-foreground/70"
          >
            Automatically generate high-fidelity flowcharts from your code in seconds. Built for architects and developers aiming for structural clarity.
          </motion.p>
          ...
          >
            ...
            <GlassButton
              ...
              variant="outline"
              size="lg"
              onClick={() => window.open('https://github.com/kumarimanjusrimohantycse2024-art/FlowZen', '_blank')}
            />
          </motion.div>
          ...
          <motion.div
            ...
            className="grid gap-4 rounded-2xl border border-border/30 bg-background/60 p-4 sm:p-6 backdrop-blur-sm dark:border-border/60 dark:bg-background/70 sm:grid-cols-3"
          >
            {heroStats.map((stat) => (
              <motion.div
                key={stat.label}
